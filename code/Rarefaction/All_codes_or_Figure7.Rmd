...
#data inspection
```{r}
geno_full_ori <- read.csv("final_data.csv") 
geno_ld_ori <- read.csv("rarefaction_LD_data.csv")
head(geno_full_ori,20)
head(geno_ld_ori,20)
...
geno_full_ori[is.na(geno_full_ori)] <- -9
...
formatted_geno_full <- rbind(geno_full_copy1,geno_full_copy2) %>% arrange(Sample) 
write.table(formatted_geno_full,"formatted_geno_full.txt")
...
unique(geno_ld_copy1$solcap_snp_sl_100003)
...
geno_ld_copy2 <- geno_ld_ori  %>%
 ...
 unique(geno_ld_copy2$solcap_snp_sl_100003)
 
formatted_geno_ld <- rbind(geno_ld_copy1,geno_ld_copy2) %>% arrange(Sample)
write.table(formatted_geno_ld,"formatted_geno_full_ld.txt")
...
adze_full <- geno_plus_group_full[,c(1,3343,2:3342)] 
head(adze_full,20)
...
```{r}
full_result_r_ori <- read.table("./madmaize_r.txt")
full_result_p_ori <- read.table("./madmaize_p.txt")
filtered_result_r_ori <- read.table("./madmaize_filtered_r.txt")
filtered_result_p_ori <- read.table("./madmaize_filtered_p.txt")

full_result_r <- select(full_result_r_ori,c(1,2,4)) 
...
full_result_r[,2] <- full_result_r[,2]/2

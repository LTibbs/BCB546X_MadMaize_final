---
title: "README"
author: "Ben Cortes and Laura Tibbs"
...
# NOTE FOR BEN--this result is just like yours, so you can keep your work here if you want :) Mine's faster, though, so at least remember it for future reference?
...
# NOTE FOR BEN--when not commented out, this shows that my result is just like yours, so you can keep your work here too :)
# also, you can use this format to check if your stuff is actually the same as mine--all.equal is useful for that
...
### NOTE FOR BEN-- because I stored my data in lists rather than having to read it back in (see my version of dataprocessing for LPmerge, above), I don't have to make the manual EXPEN_list and EXIM_list like you did to run the loop. Again, this is a place yours wasn't wrong, but I just changed it to make it easier and more coded rather than manual, and as a double-check
...
# NOTE for BEN--making a vector of the best consensus maps makes it a little more code-friendly if you want to try that. Again, this part is not wrong, just a change.
...
# There is a problem here; not all SNPs in my data are in the consensus map. In fact, there are 6471 SNPs in my data but only 5296 SNPs in the consensus map.
